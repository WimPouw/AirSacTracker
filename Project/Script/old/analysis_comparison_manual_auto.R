...
install_load("tidyverse", "effsize", "psych", "signal", "foreach")
...
for (a in 1: length(list_of_files)){
  ...
  hough_radius <- hough_radius %>% 
    mutate(smoothed_hough_radius = butter.it(hough_radius$r,
                                             samplingrate =  25,
                                             order = 2,
                                             lowpasscutoff = 10))
  ...
  cor_r_sm <- corr.test(joined_radii$smoothed_hough_radius, joined_radii$radius_man)
...
colnames(df) <- c("videoname", "cor_radius","cor_radius_smoothed", "cor_x",  "cor_y", 
                  "alpha","beta","thresh_div1","thresh_div2","dilation",
                  "medianblur", "nr_frames_used", "nr_frames_in_video")

saveRDS(df, file = "hough_vs_manuallytracked_radii_correlation_all_parameter_combinations.rds")
